 ...
 */
config_t *config_get() { return &my_config; }
...
int config_save(const char *filename) {
  ...
  fprintf(fp,
          "#fx binary filter treshold (0x0 to 0xff): default=0x7f (50%%)\n");
...
int config_load(const char *filename) {
  ...
  while (fgets(bufr, MAXLINE, fp) != NULL) {
    ...
    if (!token || !value || strlen(token) < 1 || strlen(value) < 1) {
      fprintf(stderr,
              "GUVCVIEW: (config) skiping invalid config entry at line %i\n",
              line);
      if (token)
        free(token);
      if (value)
    ...
    if (strcmp(token, "width") == 0)
      ...
      fprintf(
          stderr,
          "GUVCVIEW: (config) skiping invalid entry at line %i ('%s', '%s')\n",
          line, token, value);
  ...
  // line, strerror(errno)); 	fclose(fp); 	return -1;
  // }
