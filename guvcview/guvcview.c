...
int main(int argc, char *argv[]) {
  ...
  if (debug_level > 1)
    printf("GUVCVIEW: language catalog=> dir:%s type:%s cat:%s.mo\n", lc_dir,
           lc_all, txtdom);
  ...
  if (strcasecmp(my_config->render, "none") == 0)
    ...
    render = RENDER_SDL;
    printf(
        "GUVCVIEW: not build with sfml support (rebuild with --enable-sfml)\n");
...
/*select default gui API*/
  ...
  if (strncasecmp(my_config->gui, "none", 4) == 0)
    ...
    gui = GUI_GTK3;
  } else if (strncasecmp(my_config->gui, "qt6", 3) == 0) {
    fprintf(stderr,
            "Guvcview Qt interface foi removida; usando GTK3 automaticamente\n");
    gui = GUI_GTK3;
    strncpy(my_config->gui, "gtk3", 5);
  } else {
    fprintf(stderr, "GUI '%s' desconhecida; padrão GTK3 será utilizado\n",
            my_config->gui);
    gui = GUI_GTK3;
    strncpy(my_config->gui, "gtk3", 5);
  }
    ...
    fprintf(stderr, "Guvcview foi compilado sem suporte a GUI\n");
    gui = GUI_NONE;
  ...
  if (debug_level > 0)
    printf("Guvcview: using GUI %i for option %c%c%c\n", gui, my_config->gui[0],
           my_config->gui[1], my_config->gui[2]);
  ...
  if (vcodec_ind < 0) {
    char message[50];
    snprintf(message, 49, "invalid video codec '%s' using raw input",
             my_config->video_codec);
  ...
  if (acodec_ind < 0) {
    char message[50];
    snprintf(message, 49, "invalid audio codec '%s' using pcm input",
             my_config->audio_codec);
    gui_error("Guvcview warning", message, 0);
  ...
  audio_context_t *audio_ctx =
      create_audio_context(audio, my_config->audio_device);

  if (audio_ctx != NULL)
    my_config->audio_device = audio_get_device_index(audio_ctx);
  else
    fprintf(stderr, "GUVCVIEW: couldn't get a valid audio context for the "
                    "selected api - disabling audio\n");
  ...
  if (!my_options->control_panel) {
    ...
    if (ret != E_OK) {
      ...
      if (ret != E_OK) {
        ...
        gui_error("Guvcview error",
                  "could not start a video stream in the device", 1);
      }
    }
