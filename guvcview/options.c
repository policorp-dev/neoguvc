...
static opt_values_t opt_values[] = {
    ...
    {.opt_short = 'm',
     ...
     .opt_help =
         N_("Set render window flags (e.g none; full; max; WIDTHxHEIGHT)")},
    {.opt_short = 'a',
     .opt_long = "audio",
     .req_arg = 1,
    ...
    {.opt_short = 'u',
     ...
     .req_arg = 1,
     .opt_help_arg = N_("CODEC"),
     .opt_help = N_("Video codec [raw mjpg mpeg flv1 wmv1 mpg2 mp43 dx50 h264 "
                    "vp80 theo]")},
    {.opt_short = 'p',
     .opt_long = "profile",
     .req_arg = 1,
...
static options_t my_options = {.verbosity = 0,
                               .device = "/dev/video0",
                               .width = 0,
                               .height = 0,
                               .control_panel = 0,
 ...
 */
options_t *options_get() { return &my_options; }
 ...
 * returns: none
 */
void opt_print_version() { printf("Guvcview version %s\n", VERSION); }
  ...
  while ((opt = getopt_long(argc, argv, opt_string, long_options,
    ...
    case 'd': {
      ...
      if (str_size > 1) /*device needs at least 2 chars*/
        ...
        fprintf(stderr, "V4L2_CORE: (options) Error in device usage: "
                        "-d[--device] DEVICENAME \n");
      break;
    }
    case 'z': {
    ...
    case 'x':
      ...
      if (*stopstring != 'x') {
        fprintf(stderr, "V4L2_CORE: (options) Error in resolution usage: "
    ...
    case 'm': {
      ...
      if (str_size <= 4) /*[none, full, max] is at most 4 chars*/
        ...
        if (my_options.render_width <= 0 || *stopstring != 'x')
          fprintf(stderr, "V4L2_CORE: (options) Error in render_window usage: "
                          "-m[--render_window=] none|full|max|WIDTHxHEIGHT \n");
        else {
          ++stopstring;
