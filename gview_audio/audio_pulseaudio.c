...
static void pa_sourcelist_cb(pa_context *c, const pa_source_info *l, int eol,
  ...
  if (l->monitor_of_sink == PA_INVALID_INDEX) {
    ...
    if (audio_ctx->list_devices == NULL) {
      fprintf(stderr,
              "AUDIO: FATAL memory allocation failure (pa_sourcelist_cb): %s\n",
              strerror(errno));
      exit(-1);
    }
    /*fill device data*/
    audio_ctx->list_devices[audio_ctx->num_input_dev - 1].id =
        l->index; /*saves dev id*/
    strncpy(audio_ctx->list_devices[audio_ctx->num_input_dev - 1].name, l->name,
            511);
    strncpy(audio_ctx->list_devices[audio_ctx->num_input_dev - 1].description,
            l->description, 255);
...
static int pa_get_devicelist(audio_context_t *audio_ctx) {
  ...
  if (pa_context_connect(pa_ctx, NULL, 0, NULL) < 0) {
    fprintf(stderr, "AUDIO: PULSE - unable to connect to server: "
...
static void stream_request_cb(pa_stream *s, size_t length, void *data) {
  ...
  if (audio_ctx->channels == 0) {
    fprintf(stderr,
            "AUDIO: (pulseaudio) stream_request_cb failed: channels = 0\n");
    return;
  }

  if (audio_ctx->samprate == 0) {
    fprintf(stderr,
            "AUDIO: (pulseaudio) stream_request_cb failed: samprate = 0\n");
    return;
  }
...
static void *pulse_read_audio(void *data) {
   ...
   */
  // pa_context_rttime_new(pa_ctx, pa_rtclock_now() + TIME_EVENT_USEC,
  // time_event_callback, NULL);
  ...
  if (!recordstream)
    fprintf(stderr,
            "AUDIO: (pulseaudio) pa_stream_new failed (chan:%d rate:%d)\n",
            ss.channels, ss.rate);

  /* define the callbacks */
  pa_stream_set_read_callback(recordstream, stream_request_cb,
                              (void *)audio_ctx);
  ...
  if (audio_ctx->latency > 0) {
    bufattr.fragsize = bufattr.tlength =
        pa_usec_to_bytes((audio_ctx->latency * 1000) * PA_USEC_PER_MSEC, &ss);
...
int audio_init_pulseaudio(audio_context_t *audio_ctx) {
  ...
  if (pa_get_devicelist(audio_ctx) < 0) {
    fprintf(stderr, "AUDIO: Pulseaudio failed to get audio device list from "
                    "PULSE server\n");
    return -1;
  }
