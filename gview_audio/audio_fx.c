...
static void audio_fx_init() {
  ...
  if (aud_fx == NULL) {
    fprintf(stderr,
            "AUDIO: FATAL memory allocation failure (audio_fx_init): %s\n",
            strerror(errno));
    exit(-1);
  }
  /*Echo effect data */
 ...
 */
static void Butt(fx_filt_data_t *FILT, sample_t *Buff, int NumSamples,
                 int channels) {
  int index = 0;

  for (index = 0; index < NumSamples; index = index + channels) {
    sample_t out = FILT->a1 * Buff[index] + FILT->a2 * FILT->buff_in1[0] +
                   FILT->a3 * FILT->buff_in1[1] -
                   FILT->b1 * FILT->buff_out1[0] -
...
/*
 * HP Filter: out(n) = a1 * in + a2 * in(n-1) + a3 * in(n-2) - b1*out(n-1) -
 * b2*out(n-2) f - cuttof freq., from ~0 Hz to SampleRate/2 - though many synths
 * seem to filter only  up to SampleRate/4 r  = rez amount, from sqrt(2) to ~
 * 0.1
 *
...
static void HPF(audio_context_t *audio_ctx, sample_t *data, int cutoff_freq,
  ...
  if (aud_fx->HPF == NULL) {
    ...
    aud_fx->HPF->a1 = 1.0 / (1.0 + (res * aud_fx->HPF->c) +
                             (aud_fx->HPF->c * aud_fx->HPF->c));
    aud_fx->HPF->a2 = -2.0 * aud_fx->HPF->a1;
    aud_fx->HPF->a3 = aud_fx->HPF->a1;
    aud_fx->HPF->b1 =
        2.0 * ((aud_fx->HPF->c * aud_fx->HPF->c) - 1.0) * aud_fx->HPF->a1;
    aud_fx->HPF->b2 =
        (1.0 - (res * aud_fx->HPF->c) + (aud_fx->HPF->c * aud_fx->HPF->c)) *
        aud_fx->HPF->a1;
  }
...
/*
 * LP Filter: out(n) = a1 * in + a2 * in(n-1) + a3 * in(n-2) - b1*out(n-1) -
 * b2*out(n-2) f - cuttof freq., from ~0 Hz to SampleRate/2 - though many synths
 * seem to filter only  up to SampleRate/4 r  = rez amount, from sqrt(2) to ~
 * 0.1
 *
 ...
 * returns: none
 */
static void LPF(audio_context_t *audio_ctx, sample_t *data, float cutoff_freq,
                float res) {
  if (aud_fx->LPF1 == NULL) {
    ...
    aud_fx->LPF1->a1 = 1.0 / (1.0 + (res * aud_fx->LPF1->c) +
                              (aud_fx->LPF1->c * aud_fx->LPF1->c));
    aud_fx->LPF1->a2 = 2.0 * aud_fx->LPF1->a1;
    aud_fx->LPF1->a3 = aud_fx->LPF1->a1;
    aud_fx->LPF1->b1 =
        2.0 * (1.0 - (aud_fx->LPF1->c * aud_fx->LPF1->c)) * aud_fx->LPF1->a1;
    aud_fx->LPF1->b2 =
        (1.0 - (res * aud_fx->LPF1->c) + (aud_fx->LPF1->c * aud_fx->LPF1->c)) *
        aud_fx->LPF1->a1;
  }
 ...
 * returns: none
 */
static void CombFilter4(audio_context_t *audio_ctx, sample_t *data,
                        int delay1_ms, int delay2_ms, int delay3_ms,
                        int delay4_ms, float gain1, float gain2, float gain3,
                        float gain4, float in_gain) {
  int samp = 0;
  ...
  if (aud_fx->COMB4 == NULL) {
    ...
    if (aud_fx->COMB4 == NULL) {
      fprintf(stderr,
              "AUDIO: FATAL memory allocation failure (CombFilter4): %s\n",
              strerror(errno));
      exit(-1);
    }
    /*buff_size in samples*/
    ...
    aud_fx->COMB4->CombBuff10 =
        calloc(aud_fx->COMB4->buff_size1, sizeof(sample_t));
    ...
    aud_fx->COMB4->CombBuff20 =
        calloc(aud_fx->COMB4->buff_size2, sizeof(sample_t));
    ...
    aud_fx->COMB4->CombBuff30 =
        calloc(aud_fx->COMB4->buff_size3, sizeof(sample_t));
    ...
    aud_fx->COMB4->CombBuff40 =
        calloc(aud_fx->COMB4->buff_size4, sizeof(sample_t));
  ...
  for (samp = 0; samp < audio_ctx->capture_buff_size;
       samp = samp + audio_ctx->channels) {
    sample_t out1 =
        in_gain * data[samp] +
    ...
    aud_fx->COMB4->CombBuff10[aud_fx->COMB4->CombIndex1] =
        data[samp] +
        gain1 * aud_fx->COMB4->CombBuff10[aud_fx->COMB4->CombIndex1];
    aud_fx->COMB4->CombBuff20[aud_fx->COMB4->CombIndex2] =
        data[samp] +
        gain2 * aud_fx->COMB4->CombBuff20[aud_fx->COMB4->CombIndex2];
    aud_fx->COMB4->CombBuff30[aud_fx->COMB4->CombIndex3] =
        data[samp] +
        gain3 * aud_fx->COMB4->CombBuff30[aud_fx->COMB4->CombIndex3];
    aud_fx->COMB4->CombBuff40[aud_fx->COMB4->CombIndex4] =
        data[samp] +
        gain4 * aud_fx->COMB4->CombBuff40[aud_fx->COMB4->CombIndex4];
    ...
    if (audio_ctx->channels > 1) {
      ...
      aud_fx->COMB4->CombBuff11[aud_fx->COMB4->CombIndex1] =
          data[samp + 1] +
          gain1 * aud_fx->COMB4->CombBuff11[aud_fx->COMB4->CombIndex1];
      aud_fx->COMB4->CombBuff21[aud_fx->COMB4->CombIndex2] =
          data[samp + 1] +
          gain2 * aud_fx->COMB4->CombBuff21[aud_fx->COMB4->CombIndex2];
      aud_fx->COMB4->CombBuff31[aud_fx->COMB4->CombIndex3] =
          data[samp + 1] +
          gain3 * aud_fx->COMB4->CombBuff31[aud_fx->COMB4->CombIndex3];
      aud_fx->COMB4->CombBuff41[aud_fx->COMB4->CombIndex4] =
          data[samp + 1] +
          gain4 * aud_fx->COMB4->CombBuff41[aud_fx->COMB4->CombIndex4];
    ...
    if (++(aud_fx->COMB4->CombIndex1) >= aud_fx->COMB4->buff_size1)
      aud_fx->COMB4->CombIndex1 = 0;
    if (++(aud_fx->COMB4->CombIndex2) >= aud_fx->COMB4->buff_size2)
      aud_fx->COMB4->CombIndex2 = 0;
    if (++(aud_fx->COMB4->CombIndex3) >= aud_fx->COMB4->buff_size3)
      aud_fx->COMB4->CombIndex3 = 0;
    if (++(aud_fx->COMB4->CombIndex4) >= aud_fx->COMB4->buff_size4)
      aud_fx->COMB4->CombIndex4 = 0;
  }
}
 ...
 * returns: none
 */
static void all_pass(fx_delay_data_t *AP, sample_t *Buff, int NumSamples,
                     int channels, float gain) {
  int samp = 0;
  float inv_gain = 1.0 / gain;

  for (samp = 0; samp < NumSamples; samp += channels) {
    AP->delayBuff1[AP->delayIndex] =
        Buff[samp] + (gain * AP->delayBuff1[AP->delayIndex]);
    Buff[samp] =
        ((AP->delayBuff1[AP->delayIndex] * (1 - gain * gain)) - Buff[samp]) *
        inv_gain;
    if (channels > 1) {
      AP->delayBuff2[AP->delayIndex] =
          Buff[samp + 1] + (gain * AP->delayBuff2[AP->delayIndex]);
      Buff[samp + 1] = ((AP->delayBuff2[AP->delayIndex] * (1 - gain * gain)) -
                        Buff[samp + 1]) *
                       inv_gain;
    }

    if (++(AP->delayIndex) >= AP->buff_size)
      AP->delayIndex = 0;
  }
}
 ...
 * returns: none
 */
static void all_pass1(audio_context_t *audio_ctx, sample_t *data, int delay_ms,
                      float gain) {
  if (aud_fx->AP1 == NULL) {
    aud_fx->AP1 = calloc(1, sizeof(fx_delay_data_t));
    if (aud_fx->AP1 == NULL) {
      fprintf(stderr,
              "AUDIO: FATAL memory allocation failure (all_pass1): %s\n",
              strerror(errno));
      exit(-1);
    }
    ...
    if (audio_ctx->channels > 1) {
      aud_fx->AP1->delayBuff2 =
          calloc(aud_fx->AP1->buff_size, sizeof(sample_t));
  ...
  all_pass(aud_fx->AP1, data, audio_ctx->capture_buff_size, audio_ctx->channels,
           gain);
}
 ...
 * returns: none
 */
static void change_rate_less(fx_rate_data_t *RT, sample_t *Buff, int rate,
 ...
 *   rate: 2 -> [w1..w2][w1..w2][w2..w3][w2..w3]  3->
 * [w1..w2][w1..w2][w1..w2][w2..w3][w2..w3][w2..w3] args: audio_ctx - audio
 * context data - audio buffer to be processed rate -rate of added windows
 ...
 * returns: none
 */
static void change_tempo_more(audio_context_t *audio_ctx, sample_t *data,
                              int rate, int wtime_ms) {
  int samp = 0;
  int i = 0;
  int r = 0;
  int index = 0;
  ...
  if (aud_fx->RT1->wBuff1 == NULL) {
    ...
    if (aud_fx->RT1->wBuff1 == NULL) {
      fprintf(
          stderr,
          "AUDIO: FATAL memory allocation failure (change_tempo_more): %s\n",
          strerror(errno));
      exit(-1);
    }
    if (audio_ctx->channels > 1) {
 ...
 * returns: none
 */
static void audio_fx_echo(audio_context_t *audio_ctx, sample_t *data,
  ...
  if (aud_fx->ECHO == NULL) {
    ...
    if (aud_fx->ECHO == NULL) {
      fprintf(stderr,
              "AUDIO: FATAL memory allocation failure (audio_fx_echo): %s\n",
              strerror(errno));
      exit(-1);
    }
    aud_fx->ECHO->buff_size = (int)delay_ms * audio_ctx->samprate * 0.001;
    aud_fx->ECHO->delayBuff1 =
        calloc(aud_fx->ECHO->buff_size, sizeof(sample_t));
  ...
  for (samp = 0; samp < audio_ctx->capture_buff_size;
    ...
    aud_fx->ECHO->delayBuff1[aud_fx->ECHO->delayIndex] =
        data[samp] +
    ...
    if (audio_ctx->channels > 1) {
      ...
      aud_fx->ECHO->delayBuff2[aud_fx->ECHO->delayIndex] =
          data[samp] +
          (aud_fx->ECHO->delayBuff2[aud_fx->ECHO->delayIndex] * decay);
      data[samp + 1] = clip_float(out);
    }

    if (++(aud_fx->ECHO->delayIndex) >= aud_fx->ECHO->buff_size)
      aud_fx->ECHO->delayIndex = 0;
  }
}
 ...
 * returns: none
 */
static void audio_fx_reverb(audio_context_t *audio_ctx, sample_t *data,
                            int delay_ms) {
  /*assertions*/
  assert(audio_ctx != NULL);

  /*4 parallel comb filters*/
  CombFilter4(audio_ctx, data, delay_ms, delay_ms - 5, delay_ms - 10,
              delay_ms - 15, 0.55, 0.6, 0.5, 0.45, 0.7);
 ...
 * returns: none
 */
static void audio_fx_wahwah(audio_context_t *audio_ctx, sample_t *data,
                            float freq, float startphase, float depth,
                            float freqofs, float res) {
  /*assertions*/
  assert(audio_ctx != NULL);
  ...
  if (aud_fx->wahData == NULL) {
    ...
    if (aud_fx->wahData == NULL) {
      fprintf(stderr,
              "AUDIO: FATAL memory allocation failure (audio_fx_wahwah): %s\n",
              strerror(errno));
      exit(-1);
    }
  ...
  for (samp = 0; samp < audio_ctx->capture_buff_size; samp++) {
    ...
    if ((aud_fx->wahData->skipcount++) % lfoskipsamples == 0) {
      frequency =
          (1 + cos(aud_fx->wahData->skipcount * aud_fx->wahData->lfoskip +
                   aud_fx->wahData->phase)) *
          0.5;
    ...
    float out =
        (aud_fx->wahData->b0 * in + aud_fx->wahData->b1 * aud_fx->wahData->xn1 +
         aud_fx->wahData->b2 * aud_fx->wahData->xn2 -
         aud_fx->wahData->a1 * aud_fx->wahData->yn1 -
         aud_fx->wahData->a2 * aud_fx->wahData->yn2) /
        aud_fx->wahData->a0;
 ...
 * returns: none
 */
static void audio_fx_change_pitch(audio_context_t *audio_ctx, sample_t *data,
                                  int rate) {
  if (aud_fx->RT1 == NULL) {
    aud_fx->RT1 = calloc(1, sizeof(fx_rate_data_t));
    if (aud_fx->RT1 == NULL) {
      fprintf(stderr,
              "AUDIO: FATAL memory allocation failure (audio_fx_change_pitch): "
              "%s\n",
              strerror(errno));
      exit(-1);
    }
    ...
    aud_fx->RT1->rBuff1 = calloc(
        audio_ctx->capture_buff_size / audio_ctx->channels, sizeof(sample_t));
  ...
  change_rate_less(aud_fx->RT1, data, rate, audio_ctx->capture_buff_size,
                   audio_ctx->channels);
