...
static bmp_info_header_t mkv_codecPriv = {
    ...
    .biSizeImage =
        640 * 480 * 2, /*2 bytes per pixel (max buffer - use x3 for RGB)*/
    .biXPelsPerMeter = 0,
    .biYPelsPerMeter = 0,
    .biClrUsed = 0,
    .biClrImportant = 0};
 ...
 */
bmp_info_header_t *get_default_mkv_codecPriv() { return &mkv_codecPriv; }
...
int encoder_set_video_mkvCodecPriv(encoder_context_t *encoder_ctx) {
  ...
  if (encoder_ctx->video_codec_ind == 0) {
    ...
    case V4L2_PIX_FMT_H264: {
      ...
      if (encoder_ctx->h264_sps_size <= 0 || encoder_ctx->h264_sps == NULL) {
        fprintf(stderr,
                "ENCODER: can't store H264 codec private data: No SPS data\n");
        return 0;
      }
      if (encoder_ctx->h264_pps_size <= 0 || encoder_ctx->h264_pps == NULL) {
        fprintf(stderr,
                "ENCODER: can't store H264 codec private data: No PPS data\n");
        return 0;
      }

      /*alloc the private data*/
      size = 6 + 2 + encoder_ctx->h264_sps_size + 1 + 2 +
             encoder_ctx->h264_pps_size;
      ...
      if (encoder_ctx->enc_video_ctx->priv_data == NULL) {
        fprintf(stderr,
                ...
                "(encoder_set_video_mkvCodecPriv): %s\n",
                strerror(errno));
        exit(-1);
      }
    ...
    default: {
      ...
      mkv_codecPriv->biSizeImage =
          encoder_ctx->video_width * encoder_ctx->video_height *
          3; /*3 bytes per pixel (max buffer - use x3 for RGB)*/
  ...
  if (codec_id == AV_CODEC_ID_THEORA) {
    ...
    if (avpriv_split_xiph_headers(
            ...
            video_codec_data->codec_context->extradata_size, first_header_size,
    ...
    *tmp++ = header_len[1] % 0xff;
    ...
    listSupCodecs[real_index].mkv_codecPriv =
        encoder_ctx->enc_video_ctx->priv_data;
    ...
    ;
    mkv_codecPriv->biSizeImage =
        mkv_codecPriv->biWidth * mkv_codecPriv->biHeight *
        2; /*2 bytes per pixel (max buffer - use x3 for RGB)*/
...
int encoder_get_video_codec_ind_4cc(const char *codec_4cc) {
  ...
  for (real_index = 1; real_index < encoder_get_video_codec_list_size();
       ++real_index) {
