...
Assis <pj.assis@gmail.com>                                    # # # # This is a
heavily modified version of the matroska interface from x264        # #
Copyright (C) 2005 Mike Matsnev                                     # # # # This
...
static void mkv_put_ebml_num(mkv_context_t *mkv_ctx, uint64_t num, int bytes) {
  ...
  if (bytes < needed_bytes) {
    fprintf(stderr,
            "ENCODER: (matroska) bad requested size for ebml number: %" PRIu64
            " (%i < %i)\n",
...
static void mkv_put_ebml_uint(mkv_context_t *mkv_ctx, unsigned int elementid,
                              uint64_t val) {
  int i, bytes = 1;
  uint64_t tmp = val;
  while (tmp >>= 8)
...
static void mkv_put_ebml_float(mkv_context_t *mkv_ctx, unsigned int elementid,
                               double val) {
  mkv_put_ebml_id(mkv_ctx, elementid);
...
static void mkv_put_ebml_string(mkv_context_t *mkv_ctx, unsigned int elementid,
                                char *str) {
...
static void mkv_put_ebml_void(mkv_context_t *mkv_ctx, uint64_t size) {
  ...
  if (size < 2) {
    fprintf(stderr, "ENCODER: (matroska) wrong void size %" PRIu64 " < 2",
            size);
  }
...
static mkv_seekhead_t *mkv_start_seekhead(mkv_context_t *mkv_ctx,
  ...
  if (new_seekhead == NULL) {
    fprintf(
        stderr,
        "ENCODER: FATAL memory allocation failure (mkv_start_seekhead): %s\n",
        strerror(errno));
    exit(-1);
  }
...
static int mkv_add_seekhead_entry(mkv_seekhead_t *seekhead,
  ...
  if (seekhead->max_entries > 0 &&
      seekhead->max_entries <= seekhead->num_entries)
    return -1;

  entries = realloc(entries,
                    (seekhead->num_entries + 1) * sizeof(mkv_seekhead_entry_t));
  if (entries == NULL) {
    fprintf(stderr,
            "ENCODER: FATAL memory allocation failure "
            "(mkv_add_seekhead_entry): %s\n",
            strerror(errno));
    exit(-1);
  }

  entries[seekhead->num_entries].elementid = elementid;
  entries[seekhead->num_entries].segmentpos =
      filepos - seekhead->segment_offset;
 ...
 */
static int64_t mkv_write_seekhead(mkv_context_t *mkv_ctx,
  ...
  if (seekhead->reserved_size > 0) {
    if (io_seek(mkv_ctx->writer, seekhead->filepos) < 0) {
      fprintf(stderr,
              "ENCODER: (matroska) failed to write seekhead at pos %" PRIu64
              "\n",
              seekhead->filepos);
      currentpos = -1;
      goto fail;
  ...
  metaseek = mkv_start_ebml_master(mkv_ctx, MATROSKA_ID_SEEKHEAD,
                                   seekhead->reserved_size);
  for (i = 0; i < seekhead->num_entries; i++) {
    mkv_seekhead_entry_t *entry = &seekhead->entries[i];

    seekentry = mkv_start_ebml_master(mkv_ctx, MATROSKA_ID_SEEKENTRY,
                                      MAX_SEEKENTRY_SIZE);
...
static mkv_cues_t *mkv_start_cues(int64_t segment_offset) {
  ...
  if (cues == NULL) {
    fprintf(stderr,
            "ENCODER: FATAL memory allocation failure (mkv_start_cues): %s\n",
            strerror(errno));
    exit(-1);
  }
...
static int mkv_add_cuepoint(mkv_cues_t *cues, int stream, int64_t ts,
                            int64_t cluster_pos) {
  ...
  if (entries == NULL) {
    fprintf(stderr,
            "ENCODER: FATAL memory allocation failure (mkv_add_cuepoint): %s\n",
            strerror(errno));
    exit(-1);
  }
...
static int64_t mkv_write_cues(mkv_context_t *mkv_ctx, mkv_cues_t *cues,
                              int num_tracks) {
  ...
  for (i = 0; i < cues->num_entries; i++) {
    ...
    cuepoint = mkv_start_ebml_master(mkv_ctx, MATROSKA_ID_POINTENTRY,
    ...
    for (j = 0; j < cues->num_entries - i && entry[j].pts == pts; j++) {
      track_positions = mkv_start_ebml_master(
          mkv_ctx, MATROSKA_ID_CUETRACKPOSITION, MAX_CUETRACKPOS_SIZE);
      mkv_put_ebml_uint(mkv_ctx, MATROSKA_ID_CUETRACK, entry[j].tracknum);
      mkv_put_ebml_uint(mkv_ctx, MATROSKA_ID_CUECLUSTERPOSITION,
                        entry[j].cluster_pos);
...
static void mkv_write_codecprivate(mkv_context_t *mkv_ctx,
                                   stream_io_t *stream) {
  if (stream->extra_data_size && stream->extra_data != NULL)
    mkv_put_ebml_binary(mkv_ctx, MATROSKA_ID_CODECPRIVATE, stream->extra_data,
                        stream->extra_data_size);
}

static void mkv_write_trackdefaultduration(mkv_context_t *mkv_ctx,
                                           stream_io_t *stream) {
  if (stream->type == STREAM_TYPE_VIDEO) {
    mkv_put_ebml_uint(mkv_ctx, MATROSKA_ID_TRACKDEFAULTDURATION,
                      floor(1E9 / stream->fps));
  }
}
...
static int mkv_write_tracks(mkv_context_t *mkv_ctx) {
  ...
  ret = mkv_add_seekhead_entry(mkv_ctx->main_seekhead, MATROSKA_ID_TRACKS,
                               io_get_offset(mkv_ctx->writer));
  if (ret < 0)
    return ret;
  ...
  for (i = 0; i < mkv_ctx->stream_list_size; i++) {
    ...
    mkv_put_ebml_uint(mkv_ctx, MATROSKA_ID_TRACKFLAGLACING,
                      0); // no lacing (yet)
    ...
    if (stream->type == STREAM_TYPE_VIDEO) {
      ...
      if (codec_index < 0) {
        fprintf(stderr,
                "ENCODER: (matroska) bad video codec index for id:0x%x\n",
                stream->codec_id);
        return -1;
      }
      ...
      if (codec_index < 0) {
        fprintf(stderr,
                "ENCODER: (matroska) bad audio codec index for id:0x%x\n",
                stream->codec_id);
        return -1;
      }
    ...
    case STREAM_TYPE_VIDEO:
      mkv_put_ebml_uint(mkv_ctx, MATROSKA_ID_TRACKTYPE,
      ...
      mkv_put_ebml_uint(mkv_ctx, MATROSKA_ID_VIDEODISPLAYHEIGHT,
                        stream->height);
    ...
    case STREAM_TYPE_AUDIO:
      ...
      mkv_put_ebml_float(mkv_ctx, MATROSKA_ID_AUDIOSAMPLINGFREQ,
                         stream->a_rate);
    ...
    default:
      fprintf(stderr, "ENCODER: (matroska) Only audio and video are supported "
                      "by the Matroska muxer.\n");
      break;
    }
...
int mkv_write_header(mkv_context_t *mkv_ctx) {
   ...
   */
  mkv_ctx->main_seekhead =
      mkv_start_seekhead(mkv_ctx, mkv_ctx->segment_offset, 10);
  ...
  ret = mkv_add_seekhead_entry(mkv_ctx->main_seekhead, MATROSKA_ID_INFO,
                               io_get_offset(mkv_ctx->writer));
  if (ret < 0)
    return ret;
...
static int mkv_processh264_nalu(uint8_t *data, int size) {
  ...
  while (!last_nalu) {
    ...
    for (ep = nal_start; ep < data + size - 4; ++ep) {
      if (ep[0] == 0x00 && ep[1] == 0x00 && ep[2] == 0x00 && ep[3] == 0x01) {
...
static void mkv_write_block(mkv_context_t *mkv_ctx, unsigned int blockid,
                            int stream_index, uint8_t *data, int size,
                            uint64_t pts, int flags) {
  ...
  io_write_w8(
      mkv_ctx->writer,
      0x80 | (stream_index + 1)); // this assumes stream_index is less than 126
  io_write_wb16(mkv_ctx->writer,
                pts - mkv_ctx->cluster_pts); // pts and cluster_pts are scaled
  io_write_w8(mkv_ctx->writer, block_flags);
  io_write_buf(mkv_ctx->writer, data, size);
}

static int mkv_write_packet_internal(mkv_context_t *mkv_ctx, int stream_index,
                                     uint8_t *data, int size, int duration,
                                     uint64_t pts, int flags) {
  ...
  if (use_simpleblock)
    mkv_write_block(mkv_ctx, MATROSKA_ID_SIMPLEBLOCK, stream_index, data, size,
                    ts, flags);
  else {
    ebml_master_t blockgroup = mkv_start_ebml_master(
        mkv_ctx, MATROSKA_ID_BLOCKGROUP, mkv_blockgroup_size(size));
    mkv_write_block(mkv_ctx, MATROSKA_ID_BLOCK, stream_index, data, size, ts,
                    flags);
    if (duration)
  ...
  if (get_stream(mkv_ctx->stream_list, stream_index)->type ==
          STREAM_TYPE_VIDEO &&
      keyframe) {
    // fprintf(stderr,"mkv_ctx: add a cue point\n");
    int ret =
        mkv_add_cuepoint(mkv_ctx->cues, stream_index, ts, mkv_ctx->cluster_pos);
    if (ret < 0)
      return ret;
  }
...
static int mkv_cache_packet(mkv_context_t *mkv_ctx, int stream_index,
                            uint8_t *data, int size, int duration, uint64_t pts,
                            int flags) {

  if (mkv_ctx->pkt_buffer_list[mkv_ctx->pkt_buffer_write_index].data_size > 0) {
    if (enc_verbosity > 0)
      fprintf(stderr,
              "ENCODER: (matroska) packet buffer [%i] is in use: flushing "
              "cached data\n",
              mkv_ctx->pkt_buffer_write_index);

    int ret = mkv_write_packet_internal(
        mkv_ctx,
    ...
    if (ret < 0) {
      fprintf(stderr,
              "ENCODER: (matroska) Could not write cached audio packet\n");
      return ret;
    }
  }

  if (size >
      mkv_ctx->pkt_buffer_list[mkv_ctx->pkt_buffer_write_index].max_size) {
  ...
  if (mkv_ctx->pkt_buffer_list[mkv_ctx->pkt_buffer_write_index].data == NULL) {
    fprintf(stderr,
            "ENCODER: FATAL memory allocation failure (mkv_cache_packet): %s\n",
            strerror(errno));
    exit(-1);
  }
  ...
  memcpy(mkv_ctx->pkt_buffer_list[mkv_ctx->pkt_buffer_write_index].data, data,
         size);
...
/** public interface */
int mkv_write_packet(mkv_context_t *mkv_ctx, int stream_index, uint8_t *data,
                     int size, int duration, uint64_t pts, int flags) {
  ...
  if (stream->type == STREAM_TYPE_AUDIO)
    ret = mkv_cache_packet(mkv_ctx, stream_index, data, size, duration, ts,
                           flags);
  else
    ret = mkv_write_packet_internal(mkv_ctx, stream_index, data, size, duration,
                                    ts, flags);
...
int mkv_close(mkv_context_t *mkv_ctx) {
  ...
  if (mkv_ctx->pkt_buffer_list != NULL && mkv_ctx->pkt_buffer_list_size > 0) {
    while (mkv_ctx->pkt_buffer_list[mkv_ctx->pkt_buffer_read_index].data_size >
           0) {
      ret = mkv_write_packet_internal(
          mkv_ctx,
          mkv_ctx->pkt_buffer_list[mkv_ctx->pkt_buffer_read_index].stream_index,
  ...
  if (mkv_ctx->cues->num_entries) {
    ...
    ret = mkv_add_seekhead_entry(mkv_ctx->main_seekhead, MATROSKA_ID_CUES,
                                 cuespos);
    if (ret < 0)
      return ret;
  }
...
mkv_context_t *mkv_create_context(const char *filename, int mode) {
  ...
  if (mkv_ctx == NULL) {
    fprintf(
        stderr,
        "ENCODER: FATAL memory allocation failure (mkv_create_context): %s\n",
        strerror(errno));
    exit(-1);
  }
...
stream_io_t *mkv_add_video_stream(mkv_context_t *mkv_ctx, int32_t width,
                                  int32_t height, int32_t fps, int32_t fps_num,
                                  int32_t codec_id) {
  stream_io_t *stream =
      add_new_stream(&mkv_ctx->stream_list, &mkv_ctx->stream_list_size);
...
stream_io_t *mkv_add_audio_stream(mkv_context_t *mkv_ctx, int32_t channels,
  ...
  if (mkv_ctx->pkt_buffer_list == NULL) {
    mkv_ctx->pkt_buffer_write_index = 0;
    mkv_ctx->pkt_buffer_read_index = 0;
    mkv_ctx->pkt_buffer_list =
        calloc(mkv_ctx->pkt_buffer_list_size, sizeof(mkv_packet_buff_t));
    if (mkv_ctx->pkt_buffer_list == NULL) {
      fprintf(stderr,
              "ENCODER: FATAL memory allocation failure "
              "(mkv_add_audio_stream): %s\n",
              strerror(errno));
      exit(-1);
    }
