...
static SPacket_list_t* spkt_list = NULL;
/*
 * set verbosity
 * args:
 ...
 *   codec_ind - video codec index (-raw)
 *
 * asserts:
 *   none
 *
 * returns: none
 */
static void encoder_alloc_video_ring_buffer(int video_width, int video_height,
                                            int fps_den, int fps_num,
                                            int codec_ind) {
  ...
  if (video_ring_buffer == NULL) {
    fprintf(stderr,
            ...
            "(encoder_alloc_video_ring_buffer): %s\n",
            strerror(errno));
    exit(-1);
  }
...
void __attribute__((constructor)) gviewencoder_init() {
  ...
  fprintf(
      stderr,
      "ENCODER: Error - libavcodec version not supported (minimum 57.16)\n");
  return;
#endif
 ...
 * returns: none
 */
static void encoder_set_raw_video_input(encoder_context_t *encoder_ctx,
  ...
  case V4L2_PIX_FMT_MJPEG:
    ...
    encoder_ctx->enc_video_ctx->outbuf_size =
        (encoder_ctx->video_width * encoder_ctx->video_height) / 2;
    encoder_ctx->enc_video_ctx->outbuf =
        calloc(encoder_ctx->enc_video_ctx->outbuf_size, sizeof(uint8_t));
    if (encoder_ctx->enc_video_ctx->outbuf == NULL) {
      fprintf(
          stderr,
          "ENCODER: FATAL memory allocation failure (encoder_video_init): %s\n",
          strerror(errno));
      exit(-1);
    }
    break;
  ...
  default: {
    ...
    video_defaults->mkv_4cc =
        encoder_ctx->input_format; // v4l2_fourcc('Y','U','Y','2')
    strncpy(video_defaults->mkv_codec, "V_MS/VFW/FOURCC", 25);
    encoder_ctx->enc_video_ctx->outbuf_size = encoder_ctx->video_width *
                                              encoder_ctx->video_height *
                                              3; // max of 3 bytes per pixel
    encoder_ctx->enc_video_ctx->outbuf =
        calloc(encoder_ctx->enc_video_ctx->outbuf_size, sizeof(uint8_t));
    if (encoder_ctx->enc_video_ctx->outbuf == NULL) {
      fprintf(
 ...
 */
static encoder_video_context_t *
encoder_video_init(encoder_context_t *encoder_ctx) {
  // assertions
  assert(encoder_ctx != NULL);
  ...
  video_codec_t *video_defaults =
      encoder_get_video_codec_defaults(encoder_ctx->video_codec_ind);

  if (!video_defaults) {
    fprintf(stderr,
            "ENCODER: defaults for video codec index %i not found: using raw "
            "(direct input)\n",
  ...
  encoder_video_context_t *enc_video_ctx =
      calloc(1, sizeof(encoder_video_context_t));
  ...
  encoder_codec_data_t *video_codec_data =
      calloc(1, sizeof(encoder_codec_data_t));
  ...
  video_codec_data->codec =
      avcodec_find_encoder_by_name(video_defaults->codec_name);
  ...
  if (!video_codec_data->codec) {
    ...
    fprintf(stderr,
            "ENCODER: libav video codec (%i) not found - using raw input\n",
  ...
  video_codec_data->codec_context =
      avcodec_alloc_context3(video_codec_data->codec);
  avcodec_get_context_defaults3(video_codec_data->codec_context,
                                video_codec_data->codec);
#else
  ...
  /*allocate the SPacket_list*/
  spkt_list = spacket_list_new();
   ...
   * 1 (FF_MB_DECISION_BITS)   Select the MB mode which needs the fewest bits
   * (=vhq). 2 (FF_MB_DECISION_RD)     Select the MB mode which has the best
   * rate distortion.
   */
  ...
  //video_codec_data->codec_context->me_subpel_quality =
  //    video_defaults->subq;                                          // NEW
  ...
  //video_codec_data->codec_context->qmin =
  //    video_defaults->qmin; // best detail allowed - worst compression
  //video_codec_data->codec_context->qmax =
  //    video_defaults->qmax; // worst detail allowed - best compression
  ...
  video_codec_data->codec_context->pix_fmt =
      video_defaults->pix_fmt; // only yuv420p available for mpeg
  if (video_defaults->fps)
    video_codec_data->codec_context->time_base =
        (AVRational){1, video_defaults->fps}; // use codec properties fps
  else if (encoder_ctx->fps_den >= 5)
    video_codec_data->codec_context->time_base = (AVRational){
        encoder_ctx->fps_num,
        encoder_ctx->fps_den}; // default fps (for gspca this is 1/1)
  else
    video_codec_data->codec_context->time_base =
        (AVRational){1, 15}; // fallback to 15 fps (e.g gspca)

  if (video_defaults->gop_size > 0)
    video_codec_data->codec_context->gop_size = video_defaults->gop_size;
  else
    video_codec_data->codec_context->gop_size =
        video_codec_data->codec_context->time_base.den;
  ...
  case AV_CODEC_ID_H264: {
    ...
    av_opt_set(video_codec_data->codec_context->priv_data, "preset", "faster", 0);
    //av_dict_set(&video_codec_data->private_options, "crf", "23", 0);
    //av_dict_set(&video_codec_data->private_options, "tune", "zerolatency", 0);
  } break;
  case AV_CODEC_ID_HEVC: {
    //video_codec_data->codec_context->me_range = 57;
    if (video_codec_data->codec_context->max_b_frames > 8)
      video_codec_data->codec_context->max_b_frames = 8; // limit b frames to 8
    av_opt_set(video_codec_data->codec_context->priv_data, "preset", "medium", 0);
    //av_opt_set(video_codec_data->codec_context->priv_data, "crf", "26", 0);
    //av_opt_set(video_codec_data->codec_context->priv_data, "x265-params",
    //            "ref=1:rc-lookahead=20", 0);

  } break;
  case AV_CODEC_ID_VP8:    
  case AV_CODEC_ID_VP9: {
    av_opt_set(video_codec_data->codec_context->priv_data, "quality", "good", 0);
    av_opt_set(video_codec_data->codec_context->priv_data, "cpu-used", "-10", 0);
    av_opt_set(video_codec_data->codec_context->priv_data, "speed", "10", 0);
  } break;
  default:
    break;
  }
  ...
  int ret = 0;
  /* open codec*/
  if ((ret = avcodec_open2(video_codec_data->codec_context,
                           video_codec_data->codec,
                           &video_codec_data->private_options)) < 0) {
    fprintf(stderr,
            "ENCODER: could not open video codec (%s): %i - using raw input\n",
  ...
  if (!video_codec_data->frame) {
    fprintf(
        stderr,
        "ENCODER: FATAL av_frame_alloc failure (encoder_video_init): %s\n",
        strerror(errno));
    exit(-1);
  }
  video_codec_data->frame->pts = 0;
  video_codec_data->frame->format = video_codec_data->codec_context->pix_fmt;
  video_codec_data->frame->width = video_codec_data->codec_context->width;
  video_codec_data->frame->height = video_codec_data->codec_context->height;

  if(av_frame_get_buffer(video_codec_data->frame, 0) < 0) {
    fprintf(
        stderr,
        "ENCODER: FATAL av_frame_get_buffer failure (encoder_video_init): %s\n",
        strerror(errno));
    exit(-1);
  }
  ...
  if (video_codec_data->outpkt == NULL) {
    fprintf(
        stderr,
        "ENCODER: FATAL av_packet_alloc failure (encoder_video_init): %s\n",
        strerror(errno));
    exit(-1);
  }
  ...
  enc_video_ctx->tmpbuf =
      NULL; // no need to temp buffer input already in yu12 (yuv420p)
 ...
 */
static encoder_audio_context_t *
encoder_audio_init(encoder_context_t *encoder_ctx) {
  // assertions
  assert(encoder_ctx != NULL);
  ...
  audio_codec_t *audio_defaults =
      encoder_get_audio_codec_defaults(encoder_ctx->audio_codec_ind);
  ...
  encoder_audio_context_t *enc_audio_ctx =
      calloc(1, sizeof(encoder_audio_context_t));
  if (enc_audio_ctx == NULL) {
    fprintf(
        stderr,
        "ENCODER: FATAL memory allocation failure (encoder_audio_init): %s\n",
        strerror(errno));
    exit(-1);
  }
  ...
  audio_codec_data->codec =
      avcodec_find_encoder_by_name(audio_defaults->codec_name);
  /*if it fails try any codec with matching AV_CODEC_ID*/
  ...
  audio_codec_data->codec_context->profile =
      audio_defaults->profile; /*for AAC*/
  ...
  if (encoder_ctx->audio_channels < 2)
    av_channel_layout_copy(&(audio_codec_data->codec_context->ch_layout),
  ...
  if (best_samprate != encoder_ctx->audio_samprate) {
    fprintf(
        stderr,
        "ENCODER: audio codec doesn't support sample rate = %i (best is %i)\n",
        encoder_ctx->audio_samprate, best_samprate);
    encoder_ctx->audio_samprate = best_samprate;
  }

  audio_codec_data->codec_context->time_base =
  ...
  if (!encoder_check_audio_sample_fmt(audio_codec_data->codec,
    ...
    case AV_SAMPLE_FMT_S16:
      if (encoder_check_audio_sample_fmt(audio_codec_data->codec,
        ...
        fprintf(stderr, "ENCODER: could not open audio codec: no supported "
                        "sample format\n");
  ...
  /* open codec*/
  if (avcodec_open2(audio_codec_data->codec_context, audio_codec_data->codec,
                    NULL) < 0) {
  ...
  if (!audio_codec_data->frame) {
    fprintf(
        stderr,
        "ENCODER: FATAL av_frame_alloc failure (encoder_audio_init): %s\n",
        strerror(errno));
    exit(-1);
  }

  audio_codec_data->frame->format = audio_codec_data->codec_context->sample_fmt;
  audio_codec_data->frame->nb_samples = frame_size; 
#if LIBAVCODEC_VER_AT_LEAST(59, 0)
  av_channel_layout_copy(&audio_codec_data->frame->ch_layout,
                         &audio_codec_data->codec_context->ch_layout);
#else
  audio_codec_data->frame->channels = audio_codec_data->codec_context->channels;
  audio_codec_data->frame->channel_layout =
      audio_codec_data->codec_context->channel_layout;
#endif

  int ret = av_frame_get_buffer(audio_codec_data->frame, 0); 
  if ( ret < 0) {
    fprintf(
        stderr,
        "ENCODER: FATAL av_frame_get_buffer failure (encoder_audio_init): %i\n",
        ret);
    exit(-1);
  }
 ...
 */
int encoder_get_max_audio_sample_fmt() { return AV_SAMPLE_FMT_NB - 1; }
...
double encoder_buff_scheduler(int mode, double thresh, double max_time) {
  ...
  if (diff_ind >= th) {
    ...
    case ENCODER_SCHED_LIN: /*linear function*/
      sched_time =
          (double)(diff_ind - th) * (max_time / (video_ring_buffer_size - th));
      break;
    ...
    case ENCODER_SCHED_EXP: /*exponencial*/
    {
      ...
      if (exp > 0)
        ...
        sched_time = (double)(diff_ind - th) *
                     (max_time / (video_ring_buffer_size - th));
      break;
    }
    }
  }
 ...
 */
int encoder_get_valid_video_codecs() { return valid_video_codecs; }
 ...
 */
int encoder_get_valid_audio_codecs() { return valid_audio_codecs; }
 ...
 */
encoder_context_t *encoder_init(int input_format, int video_codec_ind,
                                int audio_codec_ind, int muxer_id,
                                int video_width, int video_height, int fps_num,
                                int fps_den, int audio_channels,
                                int audio_samprate) {
  ...
  if (encoder_ctx == NULL) {
    fprintf(
        stderr,
        "ENCODER: FATAL memory allocation failure (encoder_get_context): %s\n",
        strerror(errno));
    exit(-1);
  }
  ...
  /****************** ring buffer *****************/
  encoder_alloc_video_ring_buffer(video_width, video_height, fps_den, fps_num,
                                  video_codec_ind);
 ...
 * returns: error code
 */
int encoder_add_video_frame(uint8_t *frame, int size, int64_t timestamp,
                            int isKeyframe) {
  if (!video_ring_buffer)
    return -1;
  ...
  if (reference_pts == 0) {
    ...
    if (enc_verbosity > 0)
      printf("ENCODER: (add_video_frame) ref ts = %" PRId64 "\n", timestamp);
  }
  ...
  if (size > video_frame_max_size) {
    fprintf(
        stderr,
        "ENCODER: frame (%i bytes) larger than buffer (%i bytes): clipping\n",
...
int encoder_process_next_video_buffer(encoder_context_t *encoder_ctx) {
  ...
  encoder_ctx->enc_video_ctx->pts =
      video_ring_buffer[video_read_index].timestamp;

  /*raw (direct input)*/
  if (encoder_ctx->video_codec_ind == 0) {
    /*outbuf_coded_size must already be set*/
    encoder_ctx->enc_video_ctx->outbuf_coded_size =
        video_ring_buffer[video_read_index].frame_size;
...
int encoder_flush_video_buffer(encoder_context_t *encoder_ctx) {
  ...
  if (!buffer_count) {
    fprintf(stderr,
            "ENCODER: (flush video buffer) max processed buffers reached\n");
    return -1;
  }
...
int encoder_process_audio_buffer(encoder_context_t *encoder_ctx, void *data) {
  ...
  if (encoder_ctx->enc_audio_ctx == NULL || encoder_ctx->audio_channels <= 0)
    return -1;
...
//    else {
//      fprintf(stderr,
//              "ENCODER: Maximum of %i delayed video frames reached...\n",
//              MAX_DELAYED_FRAMES);
//      fprintf(stderr, "         write: %i read: %i\n", enc_video_ctx->write_df,
//              enc_video_ctx->read_df);
...
static int libav_get_encode(AVCodecContext *avctx, AVPacket *pkt,
                            int *got_packet) {
  int ret = 0;
  *got_packet = 0;
...
static void write_pkt_buffer (encoder_context_t *encoder_ctx, SPacket_t *spkt) {
  
  encoder_video_context_t *enc_video_ctx = encoder_ctx->enc_video_ctx;
   
  //printf("video packet pts: %li, dts:%li\n", spkt->pts, spkt->dts);

  enc_video_ctx->pts = spkt->pts;
  enc_video_ctx->dts = spkt->dts;
  enc_video_ctx->flags = spkt->flags;
  enc_video_ctx->duration = 0;
 
  if (spkt->size <= enc_video_ctx->outbuf_size)
    memcpy(enc_video_ctx->outbuf, spkt->data, spkt->size);
  else
    fprintf(stderr, "video packet size is bigger than output buffer(%i>%i)\n",
            spkt->size, enc_video_ctx->outbuf_size);

  encoder_ctx->enc_video_ctx->outbuf_coded_size = spkt->size;
...
int encoder_encode_video(encoder_context_t *encoder_ctx, void *input_frame) {
  ...
  if (encoder_ctx->video_codec_ind == 0) {
    ...
    if (outsize > enc_video_ctx->outbuf_size) {
      ...
      enc_video_ctx->outbuf =
          calloc(enc_video_ctx->outbuf_size, sizeof(uint8_t));
    }
  ...
  encoder_codec_data_t *video_codec_data =
      (encoder_codec_data_t *)enc_video_ctx->codec_data;

  int ret = 0;

  if (input_frame != NULL) {
    prepare_video_frame(video_codec_data, input_frame, encoder_ctx->video_width,
                        encoder_ctx->video_height);

    /* generate the pts based on the real frame timestamp */
    if (!enc_video_ctx->monotonic_pts) {
      video_codec_data->frame->pts = enc_video_ctx->pts;
    } else { 
      /* generate a true monotonic pts based on the codec fps */
      video_codec_data->frame->pts = last_video_pts + 
        ((video_codec_data->codec_context->time_base.num * 1000 /
          video_codec_data->codec_context->time_base.den) *
          90);
      last_video_pts = video_codec_data->frame->pts;
    }

    ret = libav_send_encode(video_codec_data->codec_context,
  ...
  if(ret == AVERROR(EAGAIN)) {
    //buffer is full 
    fprintf(stderr, "ENCODER: Error libav_send_encode: EAGAIN\n");  
  } else if (ret == AVERROR(EINVAL)) {
    fprintf(stderr, "ENCODER: Error libav_send_encode: EINVAL\n");
  } else if (ret < 0) {
    fprintf(stderr, "ENCODER: Error libav_send_encode: %i\n", ret);
  }
  ...
  while ( ret >= 0) {
    ...
    ret = libav_get_encode(video_codec_data->codec_context, pkt, &got_packet);
    if (ret == AVERROR(EAGAIN)) {
      break;
    }
    if (ret < 0) {
      fprintf(stderr, "ENCODER: Error libav_get_encode: %i\n", ret);
      break;
    }
    ...
    //lets buffer the packets to sort by pts
    SPacket_t* spkt = spacket_clone(pkt);
    
    if(video_codec_data->codec_context->codec_id == AV_CODEC_ID_HEVC)
      //order by dts
      spacket_list_add(spkt_list, spkt, 1);
    else
      //order by pts
      spacket_list_add(spkt_list, spkt, 0);  

    av_packet_unref(pkt);
  }

  if (enc_video_ctx->flush_delayed_frames) {
    SPacket_t* spkt = spacket_list_pop(spkt_list);
    while (spkt) {
      outsize = spkt->size;
      write_pkt_buffer(encoder_ctx, spkt);
      spacket_free(spkt);
      spkt = spacket_list_pop(spkt_list);
    }
      
    enc_video_ctx->flush_done = 1;
  
  } else {
    //sort the output packets by pts
    if (spkt_list->size >= 6) {
      SPacket_t* spkt = spacket_list_pop(spkt_list);
      outsize = spkt->size;
      write_pkt_buffer(encoder_ctx, spkt);
...
int encoder_encode_audio(encoder_context_t *encoder_ctx, void *audio_data) {
  ...
  encoder_codec_data_t *audio_codec_data =
      (encoder_codec_data_t *)enc_audio_ctx->codec_data;
  ...
  if (audio_codec_data->codec_context->codec_id == AV_CODEC_ID_PCM_F32LE) {
    ...
    int buffer_size = av_samples_get_buffer_size(
        ...
        audio_codec_data->codec_context->sample_fmt, align);

    if (buffer_size <= 0) {
      fprintf(stderr,
              "ENCODER: (encoder_encode_audio) PCM av_samples_get_buffer_size "
    ...
    enc_audio_ctx->duration =
        (audio_codec_data->codec_context->time_base.num * 1000 /
  ...
  if (!enc_audio_ctx->flush_delayed_frames) {
    ...
    if (buffer_size <= 0) {
      fprintf(stderr,
              "ENCODER: (encoder_encode_audio) av_samples_get_buffer_size "
              "error (%d): chan(%d) nb_samp(%d) samp_fmt(%d)\n",
              buffer_size,
#if LIBAVCODEC_VER_AT_LEAST(59, 0)
              audio_codec_data->codec_context->ch_layout.nb_channels,
...
#endif
        audio_codec_data->codec_context->sample_fmt,
        (const uint8_t *)audio_data, buffer_size, align);

    if (ret < 0) {
      fprintf(stderr,
              "ENCODER: (encoder_encode_audio) avcodec_fill_audio_frame error "
...
#endif
              audio_codec_data->frame->nb_samples,
              audio_codec_data->codec_context->sample_fmt, buffer_size);

      return outsize;
    }

    if (audio_codec_data->frame->nb_samples !=
        audio_codec_data->codec_context->frame_size) {
      fprintf(stderr,
              "ENCODER: audio frame->nb_samples(%i) != "
              "codec_context->frame_size(%i)",
              audio_codec_data->frame->nb_samples,
              audio_codec_data->codec_context->frame_size);
    }

    if (!enc_audio_ctx->monotonic_pts) /*generate a real pts based on the frame
                                          timestamp*/
      audio_codec_data->frame->pts = enc_audio_ctx->pts;
    else if (audio_codec_data->codec_context->time_base.den >
             0) /*generate a true monotonic pts based on the codec fps*/
      ...
      fprintf(stderr,
              "ENCODER: (encoder_encode_audio) couldn't generate a monotonic "
              "ts: time_base.den(%d)\n",
              audio_codec_data->codec_context->time_base.den);
    }
  }

  ret = libav_send_encode(audio_codec_data->codec_context,
                          audio_codec_data->frame);
  ...
  while (libav_get_encode(audio_codec_data->codec_context, pkt, &got_packet) >=
         0) {
    // if(pkt.pts != AV_NOPTS_VALUE)
    //	printf("ENCODER: (audio) pts:%" PRId64 " dts:%" PRId64 "\n", pkt.pts,
    // pkt.dts); else 	printf("ENCODER: (audio) no pts set\n");
...
void encoder_close(encoder_context_t *encoder_ctx) {
  fprintf(stderr, "ENCODER_CLOSE: enter ctx=%p\n", (void *)encoder_ctx);
  fflush(stderr);

  encoder_clean_video_ring_buffer();
  fprintf(stderr, "ENCODER_CLOSE: video ring buffer cleaned\n");
  fflush(stderr);

  if (!encoder_ctx) {
    fprintf(stderr, "ENCODER_CLOSE: ctx is NULL, exit\n");
    fflush(stderr);
    return;
  }
  ...
  fprintf(stderr, "ENCODER_CLOSE: video_ctx=%p audio_ctx=%p\n",
          (void *)enc_video_ctx, (void *)enc_audio_ctx);
  fflush(stderr);
  ...
  /*close video codec*/
  if (enc_video_ctx) {
    fprintf(stderr, "ENCODER_CLOSE: closing video codec context\n");
    fflush(stderr);
    ...
    free(enc_video_ctx);
    fprintf(stderr, "ENCODER_CLOSE: video context freed\n");
    fflush(stderr);
  }

  /*close audio codec*/
  if (enc_audio_ctx) {
    fprintf(stderr, "ENCODER_CLOSE: closing audio codec context\n");
    fflush(stderr);
    ...
    free(enc_audio_ctx);
    fprintf(stderr, "ENCODER_CLOSE: audio context freed\n");
    fflush(stderr);
  }

  free(encoder_ctx);
  fprintf(stderr, "ENCODER_CLOSE: encoder_ctx freed\n");
  fflush(stderr);

  fprintf(stderr, "ENCODER_CLOSE: freeing packet list %p\n", (void *)spkt_list);
  fflush(stderr);
  spacket_list_free(spkt_list);
  spkt_list = NULL;
  fprintf(stderr, "ENCODER_CLOSE: packet list freed\n");
  fflush(stderr);
  ...
  fprintf(stderr, "ENCODER_CLOSE: exit\n");
  fflush(stderr);
}
