...
static int video_init(int width, int height, int flags, int win_w, int win_h) {
  ...
  if (sdl_window == NULL) /*init SDL*/
  {
    ...
    if (!err) {
      ...
      fprintf(stderr,
              "RENDER: Couldn't determine display mode for video display %i\n",
              display_index);
  ...
  if (render_verbosity > 2) {
    ...
    if (!rend_info) {
      fprintf(stderr, "RENDER: Couldn't allocate memory for the renderer info "
                      "data structure\n");
      render_sdl2_clean();
      return -5;
    }
  ...
  if (main_renderer == NULL) {
    ...
    main_renderer = SDL_CreateRenderer(
        sdl_window, -1, SDL_RENDERER_TARGETTEXTURE | SDL_RENDERER_SOFTWARE);
  ...
  rending_texture =
      SDL_CreateTexture(main_renderer, SDL_PIXELFORMAT_IYUV, /*yuv420p*/
