...
static struct uvc_menu_info led_menu_entry[4] = {
    {0, N_("Off")}, {1, N_("On")}, {2, N_("Blinking")}, {3, N_("Auto")}};
...
uint8_t get_guid_unit_id(v4l2_dev_t *vd, uint8_t *guid) {
  ...
  if (my_device_list->list_devices[vd->this_device].vendor != 0x046D) {
    if (verbosity > 2)
      printf("V4L2_CORE: not a logitech device (vendor_id=0x%4x): skiping "
  ...
  if (verbosity > 2)
    printf("V4L2_CORE: checking pan/tilt unit id for device %i (bus:%" PRId64
           " dev:%" PRId64 ")\n",
  ...
  for (i = 0; i < cnt; i++) {
    ...
    if (verbosity > 2)
      printf("V4L2_CORE: (libusb) checking bus(%" PRId64 ") dev(%" PRId64
             ") for device\n",
    ...
    if (libusb_get_device_descriptor(device, &desc) == 0) {
      for (i = 0; i < desc.bNumConfigurations; ++i) {
        ...
        if (libusb_get_config_descriptor(device, i, &config) == 0) {
          ...
          for (j = 0; j < config->bNumInterfaces; j++) {
            ...
            for (k = 0; k < config->interface[j].num_altsetting; k++) {
              ...
              if (interface->bInterfaceClass != LIBUSB_CLASS_VIDEO ||
                ...
                continue;
              ptr = interface->extra;
              while (ptr - interface->extra + sizeof(xu_descriptor) <
          ...
          fprintf(stderr,
                  "V4L2_CORE: (libusb) couldn't get config descriptor for "
                  "configuration %i\n",
                  i);
      }
    } else
  ...
  for (i = 0; i < LENGTH_OF_XU_MAP; i++) {
    ...
    if ((err = xioctl(vd->fd, UVCIOC_CTRL_MAP, &xu_mappings[i])) < 0) {
      if ((errno != EEXIST) || (errno != EACCES)) {
        ...
        fprintf(stderr, "V4L2_CORE: need admin previledges for adding "
                        "extension controls\n");
        fprintf(stderr, "V4L2_CORE: please run 'guvcview --add_ctrls' as root "
                        "(or with sudo)\n");
        return (-1);
      } else
