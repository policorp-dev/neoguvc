 ...
 * returns: error code
 */
static int query_ioctl(v4l2_dev_t *vd, int current_ctrl,
...
static void print_control(v4l2_ctrl_t *control, int i) {
  ...
  case V4L2_CTRL_TYPE_STRING:
    ...
    printf("\tmin:%d max:%d step:%d curr: %s\n", control->control.minimum,
           control->control.maximum, control->control.step, control->string);
    break;
  ...
  case V4L2_CTRL_TYPE_MENU:
    ...
    printf("\tmin:%d max:%d def:%d curr:%d\n", control->control.minimum,
           control->control.maximum, control->control.default_value,
           control->value);
  ...
  case V4L2_CTRL_TYPE_INTEGER_MENU:
    ...
    for (j = 0; control->menu[j].index <= control->control.maximum; j++)
      printf("\tmenu[%d]: [%d] -> %" PRId64 " (0x%" PRIx64 ")\n", j,
             control->menu[j].index, (int64_t)control->menu[j].value,
...
void v4l2_subscribe_control_events(v4l2_dev_t *vd, unsigned int control_id) {
  ...
  if (ret)
    fprintf(stderr,
            "V4L2_CORE: failed to subscribe events for control 0x%08x: %s\n",
 ...
 */
static v4l2_ctrl_t *add_control(v4l2_dev_t *vd,
                                struct v4l2_queryctrl *queryctrl,
                                v4l2_ctrl_t **current, v4l2_ctrl_t **first) {
  /*assertions*/
  assert(vd != NULL);
  assert(vd->fd > 0);
  ...
  if (queryctrl->flags & V4L2_CTRL_FLAG_DISABLED) {
    printf(
        "V4L2_CORE: Control 0x%08x is disabled: remove it from control list\n",
        queryctrl->id);
    return NULL;
  }
  ...
  if (queryctrl->type == V4L2_CTRL_TYPE_MENU ||
    ...
    for (querymenu.index = queryctrl->minimum;
      ...
      if (menu == NULL) {
        ...
        fprintf(
            stderr,
            "V4L2_CORE: FATAL memory allocation failure (add_control): %s\n",
            strerror(errno));
        exit(-1);
      }
  ...
  control->name =
      strdup(dgettext(GETTEXT_PACKAGE_V4L2CORE, (char *)control->control.name));
  ...
  if (control->menu != NULL && control->control.type == V4L2_CTRL_TYPE_MENU) {
    ...
    for (i = 0; i < menu_entries; i++)
      control->menu_entry[i] = strdup(
          dgettext(GETTEXT_PACKAGE_V4L2CORE, (char *)control->menu[i].name));
  ...
  if (control->control.type == V4L2_CTRL_TYPE_STRING) {
    control->string =
        (char *)calloc(control->control.maximum + 1, sizeof(char));
...
int enumerate_v4l2_control(v4l2_dev_t *vd) {
  ...
  while ((ret = query_ioctl(vd, currentctrl, &queryctrl)) == 0) {
    if (add_control(vd, &queryctrl, &current, &(vd->list_device_controls)) !=
        NULL)
      n++;
  ...
  printf("buggy V4L2_CTRL_FLAG_NEXT_CTRL flag implementation (workaround "
         "enabled)\n");
  ...
  for (currentctrl = V4L2_CID_CAMERA_CLASS_BASE;
       currentctrl < V4L2_CID_CAMERA_CLASS_BASE + 32; currentctrl++) {
    queryctrl.id = currentctrl;
    if (xioctl(vd->fd, VIDIOC_QUERYCTRL, &queryctrl) == 0) {
      if (add_control(vd, &queryctrl, &current, &(vd->list_device_controls)) !=
          NULL)
...
static void update_ctrl_flags(v4l2_dev_t *vd, int id) {
  ...
  case V4L2_CID_EXPOSURE_AUTO: {
    ...
    case V4L2_EXPOSURE_AUTO: {
      v4l2_ctrl_t *ctrl_that =
          v4l2core_get_control_by_id(vd, V4L2_CID_IRIS_ABSOLUTE);
      if (ctrl_that)
        ctrl_that->control.flags |= V4L2_CTRL_FLAG_GRABBED;

      ctrl_that = v4l2core_get_control_by_id(vd, V4L2_CID_IRIS_RELATIVE);
      if (ctrl_that)
        ctrl_that->control.flags |= V4L2_CTRL_FLAG_GRABBED;
      ctrl_that = v4l2core_get_control_by_id(vd, V4L2_CID_EXPOSURE_ABSOLUTE);
      if (ctrl_that)
        ctrl_that->control.flags |= V4L2_CTRL_FLAG_GRABBED;
    } break;
    ...
    case V4L2_EXPOSURE_APERTURE_PRIORITY: {
      v4l2_ctrl_t *ctrl_that =
          v4l2core_get_control_by_id(vd, V4L2_CID_EXPOSURE_ABSOLUTE);
      if (ctrl_that)
        ctrl_that->control.flags |= V4L2_CTRL_FLAG_GRABBED;
      ctrl_that = v4l2core_get_control_by_id(vd, V4L2_CID_IRIS_ABSOLUTE);
      if (ctrl_that)
        ctrl_that->control.flags &= !(V4L2_CTRL_FLAG_GRABBED);
      ctrl_that = v4l2core_get_control_by_id(vd, V4L2_CID_IRIS_RELATIVE);
      if (ctrl_that)
  ...
  case V4L2_CID_FOCUS_AUTO: {
    ...
    if (ctrl_this->value > 0) {
      v4l2_ctrl_t *ctrl_that =
          v4l2core_get_control_by_id(vd, V4L2_CID_FOCUS_ABSOLUTE);
      if (ctrl_that)
        ctrl_that->control.flags |= V4L2_CTRL_FLAG_GRABBED;

      ctrl_that = v4l2core_get_control_by_id(vd, V4L2_CID_FOCUS_RELATIVE);
      if (ctrl_that)
        ctrl_that->control.flags |= V4L2_CTRL_FLAG_GRABBED;
    } else {
      v4l2_ctrl_t *ctrl_that =
  ...
  case V4L2_CID_HUE_AUTO: {
    ...
    if (ctrl_this == NULL)
      break;
    if (ctrl_this->value > 0) {
      v4l2_ctrl_t *ctrl_that = v4l2core_get_control_by_id(vd, V4L2_CID_HUE);
      if (ctrl_that)
        ctrl_that->control.flags |= V4L2_CTRL_FLAG_GRABBED;
    } else {
      v4l2_ctrl_t *ctrl_that = v4l2core_get_control_by_id(vd, V4L2_CID_HUE);
  ...
  case V4L2_CID_AUTO_WHITE_BALANCE: {
    ...
    if (ctrl_this->value > 0) {
      v4l2_ctrl_t *ctrl_that =
          v4l2core_get_control_by_id(vd, V4L2_CID_WHITE_BALANCE_TEMPERATURE);
      if (ctrl_that)
        ctrl_that->control.flags |= V4L2_CTRL_FLAG_GRABBED;
      ctrl_that = v4l2core_get_control_by_id(vd, V4L2_CID_BLUE_BALANCE);
      if (ctrl_that)
        ctrl_that->control.flags |= V4L2_CTRL_FLAG_GRABBED;
      ctrl_that = v4l2core_get_control_by_id(vd, V4L2_CID_RED_BALANCE);
      if (ctrl_that)
        ctrl_that->control.flags |= V4L2_CTRL_FLAG_GRABBED;
    } else {
      v4l2_ctrl_t *ctrl_that =
    ...
    if ((current->next == NULL) || (current->next->cclass != current->cclass)) {
      ...
      if (ret) {
        ...
        if (current->cclass == V4L2_CTRL_CLASS_USER &&
          ...
          fprintf(stderr,
                  "V4L2_CORE: using VIDIOC_G_EXT_CTRLS on single controls for "
                  "class: 0x%08x\n",
                  current->cclass);
          for (i = 0; i < count; i++) {
            ctrls.count = 1;
            ...
            if (ret)
              fprintf(
                  stderr,
                  "V4L2_CORE: control id: 0x%08x failed to get (error %i)\n",
                  clist[i].id, ret);
          }
        }
      }
      ...
      for (i = 0; i < count; i++) {
        ...
        case V4L2_CTRL_TYPE_STRING: {
          ...
          if (len > max_len) {
            ...
            fprintf(stderr,
                    "V4L2_CORE: control (0x%08x) returned string size of %d "
                    "when max is %d\n",
...
int get_control_value_by_id(v4l2_dev_t *vd, int id) {
  ...
  if (control->cclass == V4L2_CTRL_CLASS_USER &&
    ...
    if (ret)
      fprintf(stderr,
              "V4L2_CORE: control id: 0x%08x failed to get value (error %i)\n",
              ctrl.id, ret);
    else
    ...
    if (control->control.type == V4L2_CTRL_TYPE_STRING) {
      ...
      if (ctrl.string == NULL) {
        fprintf(stderr,
                "V4L2_CORE: FATAL memory allocation failure "
                "(v4l2core_get_control_value_by_id): %s\n",
                strerror(errno));
        exit(-1);
      }
    }
...
void set_v4l2_control_values(v4l2_dev_t *vd) {
  ...
  for (; current != NULL; current = current->next) {
    ...
    case V4L2_CTRL_TYPE_STRING: {
      ...
      if (len > max_len) {
        ...
        if (clist[count].string == NULL) {
          fprintf(stderr,
                  "V4L2_CORE: FATAL memory allocation failure "
                  "(set_v4l2_control_values): %s\n",
                  strerror(errno));
          exit(-1);
        }
        clist[count].string =
            strncpy(clist[count].string, current->string, max_len);
        clist[count].string[max_len + 1] = 0; /*NULL terminated*/
        fprintf(stderr,
                "V4L2_CORE: control (0x%08x) trying to set string size of %d "
                "when max is %d (clip)\n",
    ...
    if ((current->next == NULL) || (current->next->cclass != current->cclass)) {
      ...
      if (ret) {
        fprintf(stderr,
                "V4L2_CORE: VIDIOC_S_EXT_CTRLS for multiple controls failed "
                "(error %i)\n",
                ret);
        struct v4l2_control ctrl;
        ...
        if (current->cclass == V4L2_CTRL_CLASS_USER &&
          ...
          for (i = 0; i < count; i++) {
            ...
            if (ret) {
              ...
              if (ctrl)
                fprintf(stderr,
                        "V4L2_CORE: control(0x%08x) \"%s\" failed to set "
                        "(error %i)\n",
                        clist[i].id, ctrl->control.name, ret);
              else
                fprintf(stderr,
                        "V4L2_CORE: control(0x%08x) failed to set (error %i)\n",
                        clist[i].id, ret);
            }
          }
        } else {
          fprintf(stderr,
                  "V4L2_CORE: using VIDIOC_S_EXT_CTRLS on single controls for "
                  "class: 0x%08x\n",
                  current->cclass);
          for (i = 0; i < count; i++) {
            ctrls.count = 1;
    ...
    case V4L2_CTRL_TYPE_STRING: {
      ...
      if (len > max_len) {
        ...
        if (ctrl.string == NULL) {
          fprintf(stderr,
                  "V4L2_CORE: FATAL memory allocation failure "
                  "(v4l2core_set_control_value_by_id): %s\n",
                  strerror(errno));
          exit(-1);
        }
