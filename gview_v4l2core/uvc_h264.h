...
/*******************************************************************************
 * bUsageType defines
 * The bUsageType used in Probe/Commit structure. The UCCONFIG parameters are
 * based on "UCConfig Modes v1.1".
 * bUsageType  UCConfig   Description
 *   4           0        Non-scalable single layer AVC bitstream with simulcast
 *                        (number of simulcast streams>=1)
 *   5           1        SVC temporal scalability with hierarchical P with
 *                        simulcast(number of simulcast streams>=1)
 *   6           2q       SVC temporal scalability + Quality/SNR scalability
 *                        with simulcast(number of simulcast streams>=1)
 *   7           2s       SVC temporal scalability + spatial scalability with
 *                        simulcast(number of simulcast streams>=1)
 *   8           3        Full SVC scalability (temporal scalability +
...
#define xLayerID(stream_id, quality_id, dependency_id, temporal_id)            \
  ((((stream_id) & 7) << 10) | (((quality_id) & 7) << 7) |                     \
   (((dependency_id) & 15) << 3) | ((temporal_id) & 7))
