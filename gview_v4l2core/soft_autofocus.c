...
static int ACweight[64] = {0, 1, 2, 3, 4, 5, 6, 7, 1, 1, 2, 3, 4, 5,
                           6, 7, 2, 2, 2, 3, 4, 5, 6, 7, 3, 3, 3, 3,
                           4, 5, 6, 7, 4, 4, 4, 4, 4, 5, 6, 7, 5, 5,
                           5, 5, 5, 5, 6, 7, 7, 7, 7, 7, 7, 7, 7, 7};

/*use insert sort by default - it's the fastest for small and almost sorted
 * arrays (our case)*/
static int sort_method =
    AUTOF_SORT_INSERT; /* 1 - Quick sort   2 - Shell sort  3- insert sort  other
                          - bubble sort*/
 ...
 * returns: none
 */
void v4l2core_soft_autofocus_set_sort(int method) { sort_method = method; }
...
int soft_autofocus_init(v4l2_dev_t *vd) {
  ...
  if (!vd->has_focus_control_id) {
    fprintf(stderr, "V4L2_CORE: can't init software autofocus - no focus "
                    "control detected\n");
  ...
  if (focus_ctx == NULL) {
    fprintf(stderr,
            ...
            "(v4l2core_soft_autofocus_init): %s\n",
            strerror(errno));
    exit(-1);
  }

  focus_ctx->focus_control =
      v4l2core_get_control_by_id(vd, vd->has_focus_control_id);
  ...
  focus_ctx->left =
      focus_ctx->f_min + focus_ctx->i_step; /*start with focus at 8*/
  focus_ctx->focus = -1;
...
/*
 * quick sort
 * (the fastest and more complex - recursive, doesn't do well on almost sorted
 * data) args: left - right -
 *
 * asserts:
 *   focus_ctx is not null
 *
...
static void q_sort(int left, int right) {
  ...
  if (l_hold < pivot)
    q_sort(l_hold, pivot - 1);
  if (r_hold > pivot)
    q_sort(pivot + 1, r_hold);
}
...
static void s_sort(int size) {
  ...
  for (gap = size / 2; gap > 0; gap /= 2) {
    for (i = gap; i <= size; i++) {
      for (j = i - gap;
           j >= 0 && (focus_ctx->arr_sharp[j] > focus_ctx->arr_sharp[j + gap]);
           j -= gap) {
...
static void i_sort(int size) {
  ...
  for (i = 1; i <= size; i++) {
    for (j = i;
         j > 0 && (focus_ctx->arr_sharp[j - 1] > focus_ctx->arr_sharp[j]);
         j--) {
...
static int focus_sort(int size) {
  if (size >= 20) {
    fprintf(stderr,
            "V4L2_CORE: (soft_autofocus) focus array size=%d exceeds 20\n",
            size);
    size = 10;
  }
 ...
 */
static int16_t *focus_extract_Y(uint8_t *frame, int16_t *dataY, int width,
                                int height) {
  int i = 0;
  uint8_t *pimg;
  pimg = frame;
...
static int checkFocus() {
  ...
  if (focus_ctx->step <= focus_ctx->i_step) {
    if (abs((focus_ctx->sharpLeft - focus_ctx->focus_sharpness) <
            (focus_ctx->focus_sharpness / TH)) &&
        (abs(focus_ctx->sharpRight - focus_ctx->focus_sharpness) <
         (focus_ctx->focus_sharpness / TH))) {
      return (FLAT);
    } else if (((focus_ctx->focus_sharpness - focus_ctx->sharpRight)) >=
                   (focus_ctx->focus_sharpness / TH) &&
               ((focus_ctx->focus_sharpness - focus_ctx->sharpLeft)) >=
                   (focus_ctx->focus_sharpness / TH)) {
      /*
      ...
      if (abs((focus_ctx->sharpLeft - focus_ctx->focus_sharpness)) >=
          (focus_ctx->focus_sharpness / TH)) {
        if (focus_ctx->sharpLeft > focus_ctx->focus_sharpness)
          left++;
        else
          right++;
      }
      if (abs((focus_ctx->sharpRight - focus_ctx->focus_sharpness)) >=
          (focus_ctx->focus_sharpness / TH)) {
        if (focus_ctx->sharpRight > focus_ctx->focus_sharpness)
          right++;
        else
          left++;
      }
    ...
    if (((focus_ctx->focus_sharpness - focus_ctx->sharpRight)) >=
            (focus_ctx->focus_sharpness / TH) &&
        ((focus_ctx->focus_sharpness - focus_ctx->sharpLeft)) >=
            (focus_ctx->focus_sharpness / TH)) {
      return (LOCAL_MAX);
...
int soft_autofocus_get_sharpness(uint8_t *frame, int width, int height, int t) {
  ...
  int numMCUy =
      height / (8 * 2); /*covers 1/2 of height- height should be even*/
  int16_t dataMCU[64];
  int16_t *data;
  ...
  if (Y == NULL) {
    fprintf(stderr,
            "V4L2_CORE: FATAL memory allocation failure "
            "(soft_autofocus_get_sharpness): %s\n",
            strerror(errno));
    exit(-1);
  }
    ...
    for (xp = 0; xp < numMCUx; xp++) {
      ...
      for (i = 0; i < 8; i++) {
        for (j = 0; j < 8; j++) {
          /*center*/
          dataMCU[i * 8 + j] =
              Y[(((height - (numMCUy - (yp * 2)) * 8) >> 1) + i) * width +
...
int soft_autofocus_get_focus_value() {
  ...
  case 0: /*sample left to right at higher step*/
    ...
    if (focus_ctx->focus >=
        focus_ctx->right) { /*get left and right from arr_sharp*/
      focus = focus_sort(focus_ctx->ind);
      /*get a window around the best value*/
      focus_ctx->left = (focus - step / 2);
      focus_ctx->right = (focus + step / 2);
      if (focus_ctx->left < focus_ctx->f_min)
        focus_ctx->left = focus_ctx->f_min;
      if (focus_ctx->right > focus_ctx->f_max)
        focus_ctx->right = focus_ctx->f_max;
    ...
    {
      focus_ctx->focus =
          focus_ctx->arr_foc[focus_ctx->ind] + step; /*next focus*/
      focus_ctx->ind++;
      focus_ctx->flag = 0;
    }
    break;
  ...
  case 1: /*sample left to right at lower step - fine tune*/
    ...
    {
      focus_ctx->focus =
          focus_ctx->arr_foc[focus_ctx->ind] + step2; /*next focus*/
      focus_ctx->ind++;
      focus_ctx->flag = 1;
    }
    break;
  ...
  focus_ctx->focus =
      (focus_ctx->focus > focus_ctx->f_max)
          ? focus_ctx->f_max
          : ((focus_ctx->focus < focus_ctx->f_min) ? focus_ctx->f_min
                                                   : focus_ctx->focus);
  focus_ctx->right =
      (focus_ctx->right > focus_ctx->f_max)
          ? focus_ctx->f_max
          : ((focus_ctx->right < focus_ctx->f_min) ? focus_ctx->f_min
                                                   : focus_ctx->right);
  focus_ctx->left =
      (focus_ctx->left > focus_ctx->f_max)
          ? focus_ctx->f_max
          : ((focus_ctx->left < focus_ctx->f_min) ? focus_ctx->f_min
                                                  : focus_ctx->left);
...
int soft_autofocus_run(v4l2_dev_t *vd, v4l2_frame_buff_t *frame) {
  ...
  if (focus_ctx->focus < 0) {
    ...
    if (v4l2core_set_control_value_by_id(
            vd, focus_ctx->focus_control->control.id) != 0)
    ...
    focus_ctx->focus_wait = (int)abs(focus_ctx->focus - focus_ctx->last_focus) *
                                1.4 / ((1000 * vd->fps_num) / vd->fps_denom) +
                            1;
    focus_ctx->last_focus = focus_ctx->focus;
  } else {
    if (focus_ctx->focus_wait == 0) {
      focus_ctx->sharpness = soft_autofocus_get_sharpness(
          frame->yuv_frame, vd->format.fmt.pix.width, vd->format.fmt.pix.height,
          5);

      if (verbosity > 1)
        printf("V4L2_CORE: (sof_autofocus) sharp=%d focus_sharp=%d foc=%d "
               "right=%d left=%d ind=%d flag=%d\n",
               focus_ctx->sharpness, focus_ctx->focus_sharpness,
               focus_ctx->focus, focus_ctx->right, focus_ctx->left,
               focus_ctx->ind, focus_ctx->flag);
      ...
      if ((focus_ctx->focus != focus_ctx->last_focus)) {
        ...
        if (v4l2core_set_control_value_by_id(
                vd, focus_ctx->focus_control->control.id) != 0)
          fprintf(stderr,
                  "V4L2_CORE: (sof_autofocus) couldn't set focus to %d\n",
                  focus_ctx->focus);

        /*number of frames until focus is stable*/
        /*1.4 ms focus time - every 1 step*/
        focus_ctx->focus_wait =
            (int)abs(focus_ctx->focus - focus_ctx->last_focus) * 1.4 /
                ((1000 * vd->fps_num) / vd->fps_denom) +
            1;
      }
      focus_ctx->last_focus = focus_ctx->focus;
    } else {
