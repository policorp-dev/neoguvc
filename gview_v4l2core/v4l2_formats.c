...
#define v4l2_pixformat2fourcc(pixelformat)                                     \
  {pixelformat & 0x000000FF, (pixelformat >> 8) & 0x000000FF,                  \
   (pixelformat >> 16) & 0x000000FF, (pixelformat >> 24) & 0x000000FF, 0}

static uint32_t decoder_supported_formats[] = {
    V4L2_PIX_FMT_YUYV, V4L2_PIX_FMT_MJPEG, V4L2_PIX_FMT_JPEG, V4L2_PIX_FMT_H264,
    V4L2_PIX_FMT_YVYU, V4L2_PIX_FMT_UYVY, V4L2_PIX_FMT_VYUY, V4L2_PIX_FMT_YYUV,
    V4L2_PIX_FMT_YUV444, V4L2_PIX_FMT_YUV555, V4L2_PIX_FMT_YUV565,
    V4L2_PIX_FMT_YUV32, V4L2_PIX_FMT_Y41P, V4L2_PIX_FMT_GREY,
    V4L2_PIX_FMT_Y10BPACK, V4L2_PIX_FMT_Y16,
#ifdef V4L2_PIX_FMT_Y16_BE
    V4L2_PIX_FMT_Y16_BE,
#endif
    V4L2_PIX_FMT_YUV420, V4L2_PIX_FMT_YUV422P, V4L2_PIX_FMT_YVU420,
    V4L2_PIX_FMT_NV12, V4L2_PIX_FMT_NV21, V4L2_PIX_FMT_NV16, V4L2_PIX_FMT_NV61,
    V4L2_PIX_FMT_NV24, V4L2_PIX_FMT_NV42, V4L2_PIX_FMT_SPCA501,
    V4L2_PIX_FMT_SPCA505, V4L2_PIX_FMT_SPCA508, V4L2_PIX_FMT_SGBRG8,
...
static int enum_frame_intervals(v4l2_dev_t *vd, uint32_t pixfmt, uint32_t width,
  ...
  vd->list_stream_formats[fmtind - 1]
      .list_stream_cap[fsizeind - 1]
      .framerate_num = NULL;
  vd->list_stream_formats[fmtind - 1]
      .list_stream_cap[fsizeind - 1]
      .framerate_denom = NULL;
  ...
  while ((ret = xioctl(vd->fd, VIDIOC_ENUM_FRAMEINTERVALS, &fival)) == 0) {
    ...
    if (fival.type == V4L2_FRMIVAL_TYPE_DISCRETE) {
      ...
      vd->list_stream_formats[fmtind - 1]
          .list_stream_cap[fsizeind - 1]
          .framerate_num = realloc(vd->list_stream_formats[fmtind - 1]
                                       .list_stream_cap[fsizeind - 1]
                                       .framerate_num,
                                   sizeof(int) * list_fps);
      if (vd->list_stream_formats[fmtind - 1]
              .list_stream_cap[fsizeind - 1]
              .framerate_num == NULL) {
        fprintf(stderr,
                "V4L2_CORE: FATAL memory allocation failure "
                "(enum_frame_intervals): %s\n",
                strerror(errno));
        exit(-1);
      }
      vd->list_stream_formats[fmtind - 1]
          .list_stream_cap[fsizeind - 1]
          .framerate_denom = realloc(vd->list_stream_formats[fmtind - 1]
                                         .list_stream_cap[fsizeind - 1]
                                         .framerate_denom,
                                     sizeof(int) * list_fps);
      if (vd->list_stream_formats[fmtind - 1]
      ...
      vd->list_stream_formats[fmtind - 1]
          .list_stream_cap[fsizeind - 1]
          .framerate_num[list_fps - 1] = fival.discrete.numerator;
      vd->list_stream_formats[fmtind - 1]
          .list_stream_cap[fsizeind - 1]
          .framerate_denom[list_fps - 1] = fival.discrete.denominator;
  ...
  if (list_fps == 0) {
    ...
    vd->list_stream_formats[fmtind - 1]
        .list_stream_cap[fsizeind - 1]
        .numb_frates = list_fps;

  if (verbosity > 0)
    printf("\n");
  if (ret != 0 && errno != EINVAL) {
    fprintf(stderr, "V4L2_CORE: (VIDIOC_ENUM_FRAMEINTERVALS) Error enumerating "
                    "frame intervals\n");
    return errno;
  }
  return 0;
...
static int enum_frame_sizes(v4l2_dev_t *vd, uint32_t pixfmt, int fmtind) {
  ...
  if (ret != 0 && errno != EINVAL) {
    fprintf(stderr, "V4L2_CORE: (VIDIOC_ENUM_FRAMESIZES) - Error enumerating "
                    "frame sizes\n");
    return errno;
...
int enum_frame_formats(v4l2_dev_t *vd) {
  ...
  int fmtind = 0;
  int valid_formats =
      0; /*number of valid formats found (with valid frame sizes)*/
  ...
  if (vd->list_stream_formats == NULL) {
    fprintf(
        stderr,
        "V4L2_CORE: FATAL memory allocation failure (enum_frame_formats): %s\n",
        strerror(errno));
    exit(-1);
  }
  ...
  while ((ret = xioctl(vd->fd, VIDIOC_ENUM_FMT, &fmt)) == 0) {
    ...
    vd->list_stream_formats = realloc(vd->list_stream_formats,
    ...
    if (dec_support && !ret)
      valid_formats++; /*the format can be decoded and it has valid frame
                          sizes*/
  }
  ...
  if (errno != EINVAL)
    fprintf(
        stderr,
        "v4L2_CORE: (VIDIOC_ENUM_FMT) - Error enumerating frame formats: %s\n",
        strerror(errno));
...
int get_format_resolution_index(v4l2_dev_t *vd, int format, int width,
  ...
  if (format >= vd->numb_formats || format < 0) {
    fprintf(stderr,
            "V4L2_CORE: [get resolution index] format index (%i) is not valid "
            "[0 - %i]\n",
