...
typedef struct _bmp_file_header_t {
  ...
  uint32_t bfOffBits;   /*Specifies the offset, in bytes,
                     ...
                     to the bitmap bits= FileHeader+InfoHeader+RGBQUAD(0 for 24bit
                     BMP)=64*/
 ...
 * returns: error code
 */
static int save_bmp(const char *filename, uint8_t *data, int width, int height,
                    int BitCount) {
  /*assertions*/
  assert(data != NULL);
  ...
  BmpFileh.bfSize =
      sizeof(bmp_file_header_t) + sizeof(bmp_info_header_t) + imgsize;
...
int save_image_bmp(v4l2_frame_buff_t *frame, const char *filename) {
  ...
  if (bmp == NULL) {
    fprintf(stderr,
            "V4L2_CORE: FATAL memory allocation failure (save_img_bmp): %s\n",
            strerror(errno));
    exit(-1);
  }
