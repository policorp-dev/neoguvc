...
#define PUTBITS                                                                \
  {                                                                            \
    ...
    if (bits_in_next_word < 0) {                                               \
      ...
      jpeg_ctx->lcode = (jpeg_ctx->lcode << (32 - jpeg_ctx->bitindex)) |       \
                        (data >> bits_in_next_word);                           \
...
static uint16_t luminance_dc_code_table[12] = {0x0000, 0x0002, 0x0003, 0x0004,
                                               0x0005, 0x0006, 0x000E, 0x001E,
                                               0x003E, 0x007E, 0x00FE, 0x01FE};

static uint16_t luminance_dc_size_table[12] = {0x0002, 0x0003, 0x0003, 0x0003,
...
static uint8_t luminance_quant_table[] = {
    0x04, 0x02, 0x03, 0x03, 0x03, 0x02, 0x04, 0x03, 0x03, 0x03, 0x04,
    0x04, 0x04, 0x04, 0x06, 0x0a, 0x06, 0x06, 0x05, 0x05, 0x06, 0x0c,
    0x08, 0x09, 0x07, 0x0a, 0x0e, 0x0c, 0x0f, 0x0f, 0x0e, 0x0c, 0x0e,
    0x0f, 0x10, 0x12, 0x17, 0x13, 0x10, 0x11, 0x15, 0x11, 0x0d, 0x0e,
    0x14, 0x1a, 0x14, 0x15, 0x17, 0x18, 0x19, 0x1a, 0x19, 0x0f, 0x13,
    0x1c, 0x1e, 0x1c, 0x19, 0x1e, 0x17, 0x19, 0x19, 0x18};

static uint8_t chrominance_quant_table[] = {
    0x04, 0x04, 0x04, 0x06, 0x05, 0x06, 0x0b, 0x06, 0x06, 0x0b, 0x18,
    0x10, 0x0e, 0x10, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18,
 ...
 * returns: none
 */
static void quantization(jpeg_encoder_ctx_t *jpeg_ctx, int16_t *const data,
                         uint16_t *const quant_table_ptr) {
  /*assertions*/
  assert(jpeg_ctx != NULL);
  assert(data != NULL);
 ...
 */
static uint8_t *huffman(jpeg_encoder_ctx_t *jpeg_ctx, uint16_t component,
                        uint8_t *output) {
  /*assertions*/
  assert(jpeg_ctx != NULL);
  assert(output != NULL);
 ...
 * returns: none
 */
static void initialization(jpeg_encoder_ctx_t *jpeg_ctx, int image_width,
                           int image_height) {
  /*assertions*/
  assert(jpeg_ctx != NULL);
  ...
  jpeg_ctx->length_minus_mcu_width =
      (uint16_t)((image_width - mcu_width) * bytes_per_pixel);
...
static uint8_t *encode_MCU(jpeg_encoder_ctx_t *jpeg_ctx, uint8_t *output) {
  ...
  quantization(jpeg_ctx, jpeg_ctx->Y2, jpeg_ctx->ILqt);
  ...
  quantization(jpeg_ctx, jpeg_ctx->CB, jpeg_ctx->ICqt);
  ...
  quantization(jpeg_ctx, jpeg_ctx->CR, jpeg_ctx->ICqt);
 ...
 * returns: pointer to output buffer
 */
static uint8_t *write_markers(jpeg_encoder_ctx_t *jpeg_ctx, uint8_t *output,
                              int huff) {
  /*assertions*/
  assert(jpeg_ctx != NULL);
  assert(output != NULL);
...
static int encode_jpeg(uint8_t *input, uint8_t *output,
  ...
  uint8_t *yuv422 = calloc(jpeg_ctx->image_width * jpeg_ctx->image_height * 2,
                           sizeof(uint8_t));
  if (yuv422 == NULL) {
    fprintf(stderr,
            "V4L2_CORE: couldn't allocate memory for jpeg encoder (fatal)\n");
    exit(-1);
  }
...
int save_image_jpeg(v4l2_frame_buff_t *frame, const char *filename) {
  ...
  if (jpeg_ctx == NULL) {
    fprintf(
        stderr,
        "V4L2_CORE: FATAL memory allocation failure (save_image_jpeg): %s\n",
        strerror(errno));
    exit(-1);
  }
  ...
  if (v4l2core_save_data_to_file(filename, jpeg, jpeg_size)) {
    fprintf(stderr,
            "V4L2_CORE: (save_image_jpeg) couldn't capture Image to %s \n",
            filename);
    ret = E_FILE_IO_ERR;
  }
